            ...
            route("/empresas") {
                ...
                get("/{filtro}") {
                    ...
                    val resultado = filtro?.toIntOrNull()?.let {
                        RepositorioEmpresa.getByEmpleadoId(it)
                ...
                // creo una nueva empresa asociada a un empleado
                post {
                    val nueva = call.receive<Empresa>()
                    val existente = RepositorioEmpresa.getAll().find {it.id == nueva.id}
                    if (existente == null ){
                        RepositorioEmpresa.create(nueva)
                        call.respond(HttpStatusCode.Created, "Empresa creada. ")
                    }
                    else{
                        call.respond(HttpStatusCode.Conflict, "Ya existe una empresa con esa ID")
                    }
                }
            }
        }
